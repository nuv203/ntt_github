 ...
 *   Client → Server : uint32_t logN, uint32_t batch, uint32_t num_runs
 *   Server → Client : uint32_t q
 *   [loop num_runs:]
 *     Client → Server : batch*N × uint32_t  (input coefficients)
 *     Server → Client : batch*N × uint32_t  (NTT results)
 *     Server → Client : uint64_t fpga_us    (FPGA-side processing time)
 */
...
int main(int argc, char **argv) {
    ...
    std::cout << "=== Correctness check (logN=10, batch=1) ===\n";
    {
        ...
        send_all(sock, &logN,     sizeof(logN));
        send_all(sock, &batch,    sizeof(batch));
        send_all(sock, &num_runs, sizeof(num_runs));

        uint32_t q = 0;
        recv_all(sock, &q, sizeof(q));

        uint32_t psi = find_psi(N, q);
        std::vector<uint32_t> pp(N); pp[0] = 1;
        for (uint32_t i = 1; i < N; i++) pp[i] = (uint32_t)(((uint64_t)pp[i-1] * psi) % q);
    ...
    for (int ci = 0; ci < NUM_CASES; ci++) {
        ...
        // Handshake: send params, receive q.
        send_all(sock, &logN,       sizeof(logN));
        send_all(sock, &batch,      sizeof(batch));
        send_all(sock, &total_runs, sizeof(total_runs));
        ...
        for (uint32_t r = 0; r < total_runs; r++) {
            // Start timer just before sending coefficients.
        ...
        /* Throughput in Mcoeff/s based on median round-trip. */
